...
let cfg = {
  ...
  paperSlippagePct:    0.5,    // 0.5% entry slippage
  paperSpreadOTM:      1.0,    // extra 1% for OTM options
  paperOrderReject:    0.02,   // 2% order rejection rate
  paperPartialFill:    0.05,   // 5% partial fill rate
  paperMarginRequired: false,
  paperMaxDailyLoss:   5000,   // ₹5000 daily loss limit
  paperBrokerage:      20,     // ₹20 per order
  paperSttPct:         0.05,   // 0.05% STT on sell
  paperGstPct:         18,     // 18% GST on brokerage
  paperLiquidityDelay: 0.10,   // 10% chance of liquidity delay
};
...
async function placeTrade(key, quote, dir, bull) {
  ...
  if (!cfg.paperMode && optionToken) {
    const liveOrder = await placeLiveOrder(key, dir, strike, lots, inst, optionToken);
    if (!liveOrder.ok) {
      console.error(`❌ [LIVE ORDER FAILED] ${liveOrder.error} — aborting trade`);
      openTrade = null;
      return;
    }
    openTrade.liveOrderId = liveOrder.orderId;
    console.log(`🟢 [LIVE ORDER] ID: ${liveOrder.orderId}`);
  }
  // ─────────────────────────────────────────────────────────────────────────────
...
// ═══════════════════════════════════════════════════════════════════════════════
// SECTION 11b2 — LIVE ORDER API (ANGEL ONE)
// ═══════════════════════════════════════════════════════════════════════════════
async function placeLiveOrder(key, dir, strike, lots, inst, optionToken) {
  try {
    const authed = await ensureAuth();
    if (!authed) return { ok: false, error: 'Auth failed' };

    const txType   = dir === 'CALL' ? 'BUY' : 'BUY'; // always BUY to open
    const qty      = lots * inst.lotSize;
    const exchange = inst.exchange;

    // Build the option symbol token (use token from option chain)
    const body = {
      variety:          'NORMAL',
      tradingsymbol:    optionToken, // Angel One uses symbolToken as tradingsymbol for options
      symboltoken:      optionToken,
      transactiontype:  txType,
      exchange:         exchange,
      ordertype:        'MARKET',
      producttype:      'CARRYFORWARD',  // NRML equivalent in Angel One
      duration:         'DAY',
      quantity:         String(qty),
      squareoff:        '0',
      stoploss:         '0',
    ...
    if (res.data?.status && res.data?.data?.orderid) {
      return { ok: true, orderId: res.data.data.orderid };
    }
    return { ok: false, error: res.data?.message || 'Order rejected' };
  } catch (e) {
    return { ok: false, error: e.response?.data?.message || e.message };
  }
}

async function exitLiveOrder(key, dir, lots, inst, optionToken) {
  try {
    const authed = await ensureAuth();
    if (!authed) return { ok: false, error: 'Auth failed' };
    ...
    const body = {
      variety:          'NORMAL',
      tradingsymbol:    optionToken,
      symboltoken:      optionToken,
      transactiontype:  'SELL',   // always SELL to close
      exchange:         exchange,
      ordertype:        'MARKET',
    ...
    return { ok: false, error: res.data?.message || 'Exit order rejected' };
  } catch (e) {
    return { ok: false, error: e.response?.data?.message || e.message };
  }
}
...
async function manageTrade() {
  ...
  // ─── TRAILING STOP LOSS ──────────────────────────────────────────────────────
  // Once premium is up 20%+ from entry, trail SL to lock in 10% profit
  if (cfg.trailingSL && exitPrem > entry * 1.20) {
    const trailSl = Math.round(exitPrem * 0.90); // trail at 10% below current
    if (trailSl > openTrade.sl) {
      console.log(`📈 [TRAIL SL] ${key} SL moved: ₹${openTrade.sl} → ₹${trailSl} (prem ₹${exitPrem})`);
      openTrade.sl = trailSl;
      io.emit('tradeUpdate', { ...openTrade, trailNote: `SL trailed to ₹${trailSl}` });
    }
  }
  // ─────────────────────────────────────────────────────────────────────────────
...
async function closeTrade(reason, exitPrem) {
  ...
  // ─── LIVE EXIT ORDER ─────────────────────────────────────────────────────────
  if (!cfg.paperMode && openTrade.optionToken) {
    const inst     = INSTRUMENTS[openTrade.key];
    const exitResp = await exitLiveOrder(
      openTrade.key, openTrade.dir,
      openTrade.filledLots || openTrade.lots,
      inst, openTrade.optionToken
    );
    if (exitResp.ok) {
      console.log(`🟢 [LIVE EXIT] Order ID: ${exitResp.orderId}`);
    } else {
      console.error(`❌ [LIVE EXIT FAILED] ${exitResp.error} — continuing close anyway`);
    }
  }
  // ─────────────────────────────────────────────────────────────────────────────
  const trade = {
...
// ═══════════════════════════════════════════════════════════════════════════════
// SECTION 12 — SMARTSTREAM WEBSOCKET (LIVE TICKS)
// ═══════════════════════════════════════════════════════════════════════════════
...
function buildSubMsg() {
  // mode 1 = LTP, exchangeType 1 = NSE, 2 = BSE
  const tokens = [];
  for (const [, inst] of Object.entries(INSTRUMENTS)) {
    tokens.push({ exchangeType: inst.exchangeType, tokens: [inst.token] });
  }
  return JSON.stringify({
    correlationID: 'dt_sub',
    action:        1,           // 1 = subscribe
    params: { mode: 1, tokenList: tokens },
  });
}

function startWebSocket() {
  if (demoMode) { wsAlive = false; return; }
  if (!authToken || authToken === 'demo-token') return;
  if (wsConn) stopWebSocket();

  try {
    wsConn = new WebSocket(SMARTSTREAM_URL, {
      headers: {
        Authorization:  `Bearer ${authToken}`,
        'x-auth-token': authToken,
        'api-key':      process.env.ANGEL_API_KEY,
        'client-code':  process.env.ANGEL_CLIENT_ID,
        'feed-token':   feedToken || authToken,
      },
    });

    wsConn.on('open', () => {
      console.log('📡 SmartStream WebSocket connected');
      wsAlive = true;
      wsReconnectCount = 0;
      io.emit('wsStatus', { alive: true });
      wsConn.send(buildSubMsg());
    ...
    wsConn.on('message', (raw) => {
      try {
        // SmartStream sends binary frames for tick data
        const buf = Buffer.isBuffer(raw) ? raw : Buffer.from(raw);
        if (buf.length < 20) return; // ping/pong or heartbeat

        // Parse SmartStream binary LTP packet
        // Byte layout (mode 1 LTP): sub_mode(1) exchType(1) token(25) seqNum(8) exchTs(8) ltp(8) ...
        const exchType  = buf.readInt8(1);
        const ltp       = buf.readInt32BE(43) / 100; // LTP in paisa → rupees
        const tokenRaw  = buf.slice(2, 27).toString('utf8').replace(/\0/g, '').trim();

        // Match token to instrument
        const match = Object.entries(INSTRUMENTS).find(([, inst]) => inst.token === tokenRaw);
        if (!match || !ltp || ltp < 10) return;

        const [key] = match;
        const prev  = priceCache[key];
        if (!prev) return;

        // Update only LTP from WS tick — keep OHLC from REST
        const updated = {
          ...prev,
          ltp,
          change:    parseFloat((ltp - prev.close).toFixed(2)),
          changePct: prev.close ? ((ltp - prev.close) / prev.close * 100).toFixed(2) : '0.00',
          stale:     false,
          ts:        Date.now(),
          wsLtp:     true,
        };
        priceCache[key] = updated;

        // Update live candle high/low/close
        if (candleCache[key]?.length > 0) {
          const last = candleCache[key][candleCache[key].length - 1];
          last.high  = Math.max(last.high, ltp);
          last.low   = Math.min(last.low, ltp);
          last.close = ltp;
        }

        // Push to UI immediately for active instrument
        if (key === activeInst) {
          io.emit('price', {
            ...updated,
          ...
          io.emit('priceBg', { key, ltp, bull: updated.bull, stale: false });
        }
      } catch (e) {
    ...
    wsConn.on('error', (e) => {
      console.error('📡 SmartStream error:', e.message);
    });

    wsConn.on('close', (code) => {
      console.warn(`📡 SmartStream closed (code ${code})`);
      wsAlive = false;
      io.emit('wsStatus', { alive: false });
      clearInterval(wsHeartbeatTimer);
      scheduleWsReconnect();
    });

  } catch (e) {
    console.error('📡 SmartStream start error:', e.message);
    scheduleWsReconnect();
  }
}
...
function scheduleWsReconnect() {
  if (wsReconnectCount >= WS_MAX_RECONNECT) {
    console.warn('📡 SmartStream max reconnects reached — REST-only mode');
    return;
  }
  const delay = Math.min(30000, 2000 * Math.pow(2, wsReconnectCount)); // exponential backoff
  wsReconnectCount++;
  console.log(`📡 SmartStream reconnecting in ${delay / 1000}s (attempt ${wsReconnectCount})`);
  wsReconnectTimer = setTimeout(() => {
    if (authToken && authToken !== 'demo-token') startWebSocket();
  }, delay);
}
...
// Live order book (for live mode verification)
app.get('/api/orders', async (req, res) => {
  if (demoMode || cfg.paperMode) return res.json({ ok: true, orders: [], note: 'Paper mode' });
  try {
    const authed = await ensureAuth();
    if (!authed) return res.json({ ok: false, error: 'Auth failed' });
    const r = await axios.get(
      'https://apiconnect.angelone.in/rest/secure/angelbroking/order/v1/getOrderBook',
      { headers: angelHeaders(), timeout: 8000 }
    );
    res.json({ ok: true, orders: r.data?.data || [] });
  } catch (e) {
...
async function initSystems() {
  ...
  // Start SmartStream WebSocket for real-time ticks
  if (!demoMode) startWebSocket();
  ...
  console.log('✅ All systems go. Tick: 1s REST + SmartStream WS | OI: 3min');
}
